...
async function init() {
  ...
  function generateAccordianItems(itemNumber, hostName) {
    ...
    for (var item = savedItems.length - 1; item >= 0; item--) {
      ...
      var savedUrl = new URL(decodeURIComponent(escape(savedItems[item][key]["url"]))); // decodeURIComponent(escape(atob(base64)))
      // Method 1
      // var encodedString = btoa(JSON.stringify(savedItems[item][key]['payload']));
      // Method 2 
      // you’re --> youâre
      var payload = savedItems[item][key]["payload"];
      var payloadStr = JSON.stringify(payload);
      var encodedString = btoa(unescape(encodeURIComponent(payloadStr)));
      // Method 3
      // const encoder = new TextEncoder();
      // const str = JSON.stringify(savedItems[item][key]['payload']);
      ...
      <a class="btn btn-link ck-viewEncodedString" encodedString="${encodedString}" accordionId="${
...
$(".btn-clipboard").on("mouseover", function () {
  $(".btn-clipboard-tooltip-span").attr(
    "data-bs-original-title",
    "Copy to clipboard!"
  );
});

// $("#toggleXFrame").on("click", function () {
//   console.log("toggle x frame");
//   console.log($(".toggleXFrame").attr("selected"));
// });
